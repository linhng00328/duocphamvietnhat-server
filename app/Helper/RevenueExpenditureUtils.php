...
class RevenueExpenditureUtils
{
    ...
    static function auto_add_expenditure_order($orderExists, $request)
    {
        ...
        if (Cache::lock('auto_add_expenditure_order' .  $orderExists->order_code, 1)->get()) {
            //tiếp tục handle
        } else {
        ...
        if (
            ...
            ||
            $orderExists->order_status == StatusDefineCode::RECEIVED_PRODUCT
        ) {
            //Phieu  chi
    ...
    static function auto_add_revenue_order_refund($orderExists, $request) //kiểm tra đơn hoàn hàng đã 
    {
        if (Cache::lock('auto_add_revenue_order_refund' .  $orderExists->order_code, 1)->get()) {
            //tiếp tục handle
        } else {
            return response()->json([
                'code' => 400,
        ...
        $total_money_refund = $orderExists->total_money_refund;
        $remaning_money_refund = $orderExists->total_final -  $total_money_refund;

        if (
            ($orderExists->order_status == StatusDefineCode::CUSTOMER_HAS_RETURNS
            ) &&  $remaning_money_refund  > 0
        ) {
            ...
            //Kiểm tra xem có phiếu thu trả hàng chưa
            $lastRE =   RevenueExpenditure::where(
                'references_value',
                $orderExists->order_code,
            )->where(
                ...
                ->where('action_create',  RevenueExpenditureUtils::ACTION_CREATE_CUSTOMER_REFUND_REVENUE,)
                ->where('is_revenue', true)->first();


            if ($lastEX != null && $lastRE == null) {

                RevenueExpenditureUtils::add_new_revenue_expenditure(
                    $request,
                    RevenueExpenditureUtils::TYPE_DEBT_COLLECTION_CUSTOMERS,
                    RevenueExpenditureUtils::RECIPIENT_GROUP_CUSTOMER,
                    $orderExists->customer_id,
                    null,
                    $orderExists->id,
    ...
    {
        if (Cache::lock('auto_add_revenue_order' .  $orderExists->order_code, 1)->get()) {
            //tiếp tục handle
        } else {
            return response()->json([
                'code' => 400,
