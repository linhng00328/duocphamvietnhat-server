...
class AdminManageUserController extends Controller
{
    ...
    public function getAll(Request $request)
    {
        $users = User::sortByRelevance(true)
            ...
            ->when(request('begin_last_visit_time') != null, function ($query) {
                $t2 =  Helper::get_begin_date_string(new Carbon(request('begin_last_visit_time')));
                $query->where('last_visit_time', '>=', $t2);
            })
            ->when(request('end_last_visit_time') != null, function ($query) {
                $t1 =  Helper::get_end_date_string(new Carbon(request('end_last_visit_time')));
                $query->where('last_visit_time', '<=', $t1);
            })
    ...
    public function updateOneUser(Request $request, $id)
    {
        ...
        if (empty($storeExists)) {
            ...
            $storeExists->update(Helper::sahaRemoveItemArrayIfNullValue(
                [
