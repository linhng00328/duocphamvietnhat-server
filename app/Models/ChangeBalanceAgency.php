...
class ChangeBalanceAgency extends BaseModel
{
    use HasFactory;
    protected $guarded = [];
    ...
    public function getTypeNameAttribute()
    {
        ...
        if ($this->type == BalanceCustomerService::ORDER_COMPLETED_T1) {
            return "Tiền chia sẻ cho đại lý người giới thiệu gián tiếp, đơn hàng " . $this->references_value . "";
        }

        if ($this->type == BalanceCustomerService::AGENCY_ORDER_COMPLETED_FOR_CUSTOMER) {
            return "Tiền đặt đơn hộ cho khách hàng, đơn hàng " . $this->references_value . "";
        }

        if ($this->type == BalanceCustomerService::ORDER_COMPLETED_CTV_RE) {
            return "Tiền hoa hồng từ khách hàng giới thiệu, đơn hàng " . $this->references_value . "";
        }
        ...
        if ($this->type == BalanceCustomerService::AGENCY_CANCEL_ORDER) {
